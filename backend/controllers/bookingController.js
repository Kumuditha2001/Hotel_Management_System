...
const updateBookingDetails = async (req, res, next) => {
  try {
    ...
    // Only the student who made the booking can edit it (admin manages status separately)
    ...
    if (booking.status !== 'Pending') {
      return res.status(400).json({
        success: false,
        message: `This booking has already been ${booking.status.toLowerCase()} and can no longer be edited`,
      });
    }
